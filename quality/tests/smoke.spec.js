...
test('chat composer sends attachment and renders file card', async ({ page }) => {
  ...
  await expect.poll(() => sentMessage).toContain('[[attachment|portfolio.pdf|');
  await expect.poll(() => sentMessage).toContain('/api/chat/attachments/chat-file.pdf]]');
  await expect(page.locator('#chatMessageList .chatAttachmentLabel')).toContainText('portfolio.pdf');
});

test('chat composer renders inline preview for image attachments', async ({ page }) => {
  let sentMessage = '';
  let chatStarted = false;
  ...
  await page.route('**/api/**', async (route) => {
    ...
    if (url.pathname.endsWith('/upload')) {
      return route.fulfill({
        ...
        contentType: 'application/json',
        body: JSON.stringify({
          fileUrl: '/files/chat-photo.jpeg',
          fileName: 'IMG_9EDC42CA-D64A-4533-8BF9-B47D15797C3E.jpeg',
        }),
      });
    }
  ...
  await page.setInputFiles('#chatFileInput', {
    name: 'IMG_9EDC42CA-D64A-4533-8BF9-B47D15797C3E.jpeg',
    mimeType: 'image/jpeg',
    buffer: Buffer.from('image-file'),
  });
  await page.click('#chatSendBtn');

  await expect.poll(() => sentMessage).toContain('[[attachment|IMG_9EDC42CA-D64A-4533-8BF9-B47D15797C3E.jpeg|');
  await expect.poll(() => sentMessage).toContain('/api/chat/attachments/chat-photo.jpeg]]');
  await expect(page.locator('#chatMessageList .chatAttachmentImage')).toHaveAttribute('src', /chat\/attachments\/chat-photo\.jpeg/);
  await expect(page.locator('#chatMessageList .chatAttachmentCaption')).toContainText('IMG_9EDC42CA-D64A-4533-8BF9-B47D15797C3E.jpeg');
});
